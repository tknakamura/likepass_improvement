...
/**
 * Runs one photo-level NPC panel (10 judges), upserts decisions,
 * recomputes aggregates, and publishes to EXPLORING when coming from NPC_REVIEWING.
 */
export async function runNpcReview(contentId: string): Promise<void> {
  ...
  if (existingCount >= NPC_JUDGE_COUNT) {
  ...
  await prisma.$transaction(async (tx) => {
    ...
    await tx.content.update({
      where: { id: contentId },
      data: {
        status: content.status === "NPC_REVIEWING" ? "EXPLORING" : content.status,
        publishedAt: content.publishedAt ?? new Date(),
      },
    });
  ...
  for (const { tagId } of content.contentTags) {
    await recalculateTagRanking(tagId);
  }

  console.log(`[npc_review_image] ${contentId} -> ${content.status === "NPC_REVIEWING" ? "EXPLORING" : content.status} (${result.decisions.length} judges)`);
}
